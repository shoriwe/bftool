if __name__ == "__main__":
    ...
    def main():
        ...
        module = import_module_from_path(arguments.module_path)
        function_ = getattr(module, arguments.function_name)
        # if arguments.queue:
        #     queue_object = getattr(module, arguments.queue)
        # else:
        #     queue_object = None
        numeric_mode = Modes.WORDLIST_BLOCK if arguments.mode == "wordlist" else Modes.BASIC_MODE

        wordlist = merge_wordlists(*get_wordlists(arguments.wordlist))

        if arguments.max_processes > 1:
            wordlists = wordlist_divider(wordlist, arguments.max_processes)
        else:
            wordlists = [wordlist]
        for sub_wordlist in wordlists:
            while active_processes.value > arguments.max_processes:
                pass
            process_handler = ProcessHandler(function_, sub_wordlist, active_threads,
